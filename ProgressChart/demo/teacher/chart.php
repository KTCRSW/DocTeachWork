...
<?php
    ...
    $sql_unfinishtask = "SELECT * FROM progress WHERE sj_id = '".$_REQUEST['sj_id']."' AND ack_std = '0' AND ack_teacher = '0' ";
...
?>
        ...
        <td ><center>
            <button type="button" class="">ยังไม่ส่ง</button>
        </center></td>
        </tr>
...
$sql_waittask = "SELECT * FROM progress WHERE sj_id = '".$_REQUEST['sj_id']."' AND ack_std = '1' AND ack_teacher = '0' ";
...
while($waittask = mysqli_fetch_array($query_waittask)){  
    $sql_std = "SELECT * FROM users WHERE no_usr = '".$waittask['usr_no_id']."'";
        $query_std = mysqli_query($con,$sql_std);
        $std_data = mysqli_fetch_array($query_std);
    if($colorflag == 0){
        ?>
